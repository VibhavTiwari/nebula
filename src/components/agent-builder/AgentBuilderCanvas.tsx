...
export function AgentBuilderCanvas({ graph, onSave }: Props) {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-panel-bg">
      {/* Left: Node Palette */}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Toolbar */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-panel-border bg-panel-card">
          ...
          <div className="flex items-center gap-3">
            {isEditingName ? (
              ...
              <button
                ...
                className="text-sm font-semibold text-text-primary hover:text-nebula-400 transition-colors flex items-center gap-2"
          ...
          <div className="nebula-tabs">
            {(["canvas", "evaluate", "code", "publish"] as const).map((view) => (
              <button
                ...
                className={clsx(
                  activeView === view ? "nebula-tab-active" : "nebula-tab"
                )}
              >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative group">
              <button className="nebula-btn-secondary text-xs flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6z" />
                </svg>
                Templates
              </button>
              <div className="absolute right-0 top-full mt-1 w-64 bg-panel-elevated border border-panel-border rounded-lg shadow-panel opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <div className="py-1">
                  {Object.entries(WORKFLOW_TEMPLATES).map(([id, tmpl]) => (
                    <button
                      key={id}
                      onClick={() => loadTemplate(id)}
                      className="w-full text-left px-3 py-2.5 text-xs hover:bg-panel-hover transition-colors"
                    >
                      <div className="font-medium text-text-primary">{tmpl.name}</div>
                      <div className="text-text-muted mt-0.5">{tmpl.description}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {workflowStatus === "draft" ? (
              ...
              <button onClick={handleUnpublish} className="nebula-btn-secondary text-xs !border-accent-orange/50 !text-accent-orange hover:!bg-accent-orange/10">
                Unpublish
              </button>
            )}
          </div>
        ...
        {/* Content area */}
        {activeView === "canvas" && (
          <div className="flex-1 nebula-canvas-bg" ref={reactFlowWrapper}>
            <ReactFlow
              nodes={nodes}
              edges={edges}
              ...
              defaultEdgeOptions={{
                animated: true,
                style: { strokeWidth: 2, stroke: "#3a3a3a" },
              }}
            >
    ...
    <div className="flex-1 flex flex-col bg-panel-bg">
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          {/* Status Card */}
          <div className="nebula-card-elevated">
            <div className="flex items-center justify-between mb-4">
              ...
              <span
                className={clsx(
                  "nebula-badge",
                  isPublished ? "nebula-badge-success" : "nebula-badge-warning"
                )}
              >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-panel-bg rounded-lg border border-panel-border">
                  <div className="text-xs text-text-muted">Workflow Name</div>
              ...
              <div className="p-4 bg-panel-bg rounded-lg border border-panel-border">
                <div className="flex items-start gap-3">
                  {isPublished ? (
                    <svg
                      className="w-5 h-5 text-accent-green mt-0.5"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                    ...
                    <svg
                      className="w-5 h-5 text-accent-yellow mt-0.5"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                {isPublished ? (
                  <button
                    onClick={onUnpublish}
                    className="nebula-btn-secondary text-sm !border-accent-orange/50 !text-accent-orange hover:!bg-accent-orange/10 flex items-center gap-2"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
          ...
          {isPublished && (
            <div className="nebula-card-elevated">
              ...
              <div className="space-y-3">
                <div className="nebula-code text-accent-purple">
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span
        className={clsx(
          "text-sm",
          checked ? "text-text-secondary" : "text-text-disabled"
        )}
      >
        {label}
      </span>
